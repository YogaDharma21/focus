...
import { format, isValid, getDaysInMonth, parseISO } from "date-fns";
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-border/40 pb-4">
                    <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">
                            <strong>Single-click</strong> box to select date & edit. <strong>Double-click</strong> box to quick-cycle mood.
                        </p>
                    </div>
                                    ...
                                    {Array.from({ length: 12 }, (_, monthIdx) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                title={`${format(new Date(selectedYear, monthIdx, dayNum), "MMM d, yyyy")}${cfg ? `: ${cfg.label} ${cfg.emoji}` : ": Empty (Single-click to select, Double-click to cycle)"}${noteObj?.text ? ` - "${noteObj.text}"` : ""}`}
                                            />
                                        );
                                    })}
                                </div>
                ...
                <div className="min-h-[44px] flex items-center justify-between p-3 rounded-xl bg-background/40 border border-border/40 text-xs">
                    {hoveredDateInfo ? (
                        <div className="flex items-center justify-between w-full">
                            ...
                            <span className="text-[10px] text-muted-foreground hidden sm:inline">Click: Select | Double-click: Cycle</span>
                        </div>
                    ) : (
                        <div className="flex items-center gap-2 text-muted-foreground">
                            <Info className="w-4 h-4 text-primary/70" />
                            <span>Single-click any box to select & edit details. Double-click to quick-cycle mood.</span>
                        </div>
                    )}
                </div>
