  ...
  return (
    <header className="h-10 w-full bg-secondary border-b border-border flex items-center justify-between px-3 select-none drag-region text-xs text-muted-foreground relative z-50 shrink-0">
      <div className="flex items-center gap-2.5 no-drag">
        ...
        {isActive && (
          <div className="flex items-center gap-1.5 ml-2 px-2.5 py-0.5 rounded-full bg-secondary border border-border text-foreground font-mono text-[10px]">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            <span className="font-semibold">{modeLabel}</span>
            <span className="text-muted-foreground">•</span>
            <span className="font-bold text-foreground">{formatTime(activeSeconds)}</span>
          </div>
        )}
      </div>
