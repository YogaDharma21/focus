...
export const MoodNotes: React.FC = () => {
  ...
  const renderMoodBadge = (moodStr: string) => {
    ...
    return (
      <span className="px-2.5 py-0.5 rounded-full bg-secondary border border-border text-foreground text-xs font-medium inline-flex items-center gap-1.5">
    ...
    <div className="h-full flex flex-col md:flex-row gap-6 p-4 md:p-6 max-w-5xl mx-auto w-full select-none overflow-hidden">
      <form onSubmit={handleSubmit} className="w-full md:w-80 shadcn-card p-5 space-y-4 shrink-0">
        <div className="flex items-center gap-2 pb-2 border-b border-border">
          ...
          <h3 className="text-xs font-semibold text-foreground">Daily Focus Reflection</h3>
        </div>

        <div className="space-y-2">
          <label className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider block">How are you feeling?</label>
        ...
        <div className="space-y-2">
          <label className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider block">Reflection & Notes</label>
          <textarea
            rows={4}
        ...
        <button
          type="submit"
          className="w-full py-2.5 rounded-lg bg-primary text-primary-foreground text-xs font-semibold hover:bg-primary/90 transition-all flex items-center justify-center gap-1.5"
        >
      ...
      <div className="flex-1 shadcn-card p-5 flex flex-col space-y-3 overflow-hidden">
        <div className="flex items-center justify-between pb-3 border-b border-border">
          <h4 className="text-xs font-semibold text-foreground">Reflection Timeline</h4>
          <span className="text-[10px] text-muted-foreground">{moodNotes.length} entries</span>
        </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-3 pr-1">
          {moodNotes.length === 0 ? (
            ...
            moodNotes.slice().reverse().map((note) => (
              <div key={note.id} className="p-4 rounded-xl bg-secondary/80 border border-border space-y-2 relative group hover:border-muted-foreground transition-colors">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="text-muted-foreground hover:text-rose-400 opacity-0 group-hover:opacity-100 transition-opacity p-1"
                    >
